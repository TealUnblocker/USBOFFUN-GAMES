...
class EnemyAI {
    ...
    start() {
        ...
        console.log(`Night ${this.game.state.currentNight} - Michael AI Config:`, this.currentEpsteinConfig);
        console.log(`Michael will spawn in ${this.currentEpsteinConfig.spawnDelay / 1000} seconds...`);
        ...
        if (this.epstein.aiLevel > 0) {
            ...
            console.log('Michael AI level is 0, not spawning');
        }
    ...
    spawnEpstein() {
        ...
        console.log('✅ Michael has spawned!');
    ...
    moveToNextLocation() {
        ...
        if (forwardLocations.length === 0 && currentDepth === 1) {
            console.log(`Michael moved: ${currentLoc} -> office`);
        ...
        if (totalProb === 0 || (forwardLocations.length === 0 && lateralLocations.length === 0 && backwardLocations.length === 0)) {
            console.log(`Michael has no valid path from ${currentLoc}`);
            return;
        }
        ...
        console.log(`Michael moved [${movementType}]: ${currentLoc} (depth ${currentDepth}) -> ${nextLocation} (depth ${this.locationDepth[nextLocation]})`);
        ...
        if (this.epstein.hasSpawned && adjacentToEp && adjacentToEp.includes(soundLocation)) {
            ...
            if (resistance > 0) {
                ...
                if (failChance < resistance) {
                    console.log(`Michael resisted the sound lure! (${resistance * 100}% chance on Night ${this.game.state.currentNight})`);
            ...
            console.log(`Michael attracted by sound: ${epCurrentLoc} (depth ${currentDepth}) -> ${soundLocation} (depth ${soundDepth})`);
            ...
            console.log(`Sound at ${soundLocation} is not adjacent to Michael at ${epCurrentLoc}`);
        }
    ...
    startHawking() {
        ...
        this.hawking.warningLevel = 0;
        console.log('Liam started at cam6');
        ...
        if (this.game.state.customNight && this.game.state.currentNight === 7) {
            ...
            console.log(`Liam AI Level ${hawkingLevel}: Initial warning in ${initialWarningTime/1000}s`);
        } else {
            // 普通夜晚使用默认20秒
    ...
    showHawkingWarning(level) {
        ...
        console.log(`Liam warning: ${level}`);
    ...
    // 霍金破坏摄像头
    hawkingBreakCamera() {
        console.log('Liam broke the camera!');
    ...
    triggerHawkingMissileJumpscare() {
        console.log('Liam missile jumpscare!');
    ...
    shockHawking() {
        if (!this.hawking.active) {
            console.log('Liam is not active');
            return false;
        }
        
        console.log('Liam shocked! Resetting timer...');
